...
let test_columns =
  ...
  >::: [
         ...
         ( "fold left2" >:: fun _ ->
           ...
           in
           let col = Column.fold_left (fun x y -> x ^ y) "0" temp in
...
let rec create_test_table columns =
  match columns with
  | [] -> Datatable.empty
  | h :: t -> A3.Datatable.append (create_test_table t) h
...
let test_datatable =
  ...
  >::: [
         ...
         ( "load_csv" >:: fun _ ->
           let test_load = load_csv test in
           let columns =
             [
               [ "LatD"; "41"; "42"; "46" ];
               [ "LatM"; "5"; "52"; "35" ];
           ...
           in
           assert_equal (create_test_table columns) test_load );
         ( "append" >:: fun _ ->
           let test_append =
             A3.Datatable.append (load_csv test) [ "LatE"; "10"; "5"; "10" ]
           in
           let columns =
             [
               [ "LatE"; "10"; "5"; "10" ];
           ...
           in
           assert_equal (create_test_table columns) test_append );
         ( "remove" >:: fun _ ->
           let test_append = A3.Datatable.remove (load_csv test) "LatD" in
           let columns =
             [ [ "LatM"; "5"; "52"; "35" ]; [ "LatS"; "59"; "48"; "59" ] ]
           in
           assert_equal (create_test_table columns) test_append );
         ( "map" >:: fun _ ->
           let test_map =
             A3.Datatable.map
               (fun x -> string_of_int (int_of_string x + 5))
               (load_csv test) "LatD"
           in
           let columns =
             [
           ...
           in
           assert_equal (create_test_table columns) test_map );
         ( "filter" >:: fun _ ->
           let test_filter =
             A3.Datatable.filter
               (fun x -> int_of_string x < 6)
               (load_csv test) "LatM"
           in
           let columns =
             [ [ "LatD"; "41" ]; [ "LatM"; "5" ]; [ "LatS"; "59" ] ]
           in
           assert_equal (create_test_table columns) test_filter );
         ( "reduce" >:: fun _ ->
           let test_reduce =
             A3.Datatable.reduce
               (fun x y -> string_of_int (int_of_string x + int_of_string y))
               (load_csv test) "0" "LatM"
           in
           assert_equal "92" test_reduce );
       ]
...
let list_simple_average_tests =
  ...
  >::: [
         ...
         ( "decreasing list 1" >:: fun _ ->
           assert_equal 2.5
             (Ema.simple_average_list (List.rev [ 1.; 2.; 3.; 4. ])) );
         ...
         ( "decreasing list 2" >:: fun _ ->
           assert_equal 3.5
             (Ema.simple_average_list (List.rev [ 2.; 3.; 4.; 5. ])) );
         ( "constant list\n   2" >:: fun _ ->
...
let list_wma_tests =
  ...
  >::: [
         ...
         ( "2\n   decreasing list for epsilon greater than 0.5" >:: fun _ ->
